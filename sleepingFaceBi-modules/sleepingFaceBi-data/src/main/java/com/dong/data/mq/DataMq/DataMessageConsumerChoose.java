...
@Component
@Slf4j
public class DataMessageConsumerChoose {
    ...
    @SneakyThrows
    @RabbitListener(queues = {MqConstant.DATA_CHOOSE_QUEUE_NAME},ackMode = "MANUAL")
    public void receiveMessage(String message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag){
        ...
        for (DataRecord DataRecord : DataRecordList) {
            ...
            Callable<String> callable = () -> {
                return qianWenData.callWithMessageChoose(DataRecordService.buildUserInput(DataRecord,DataTask.getTextType(),DataTask.getAim()).toString());
            };
...
//            try {
//                result = qianWenData.callWithMessageChoose(DataRecordService.buildUserInput(DataRecord,DataTask.getTextType(),DataTask.getAim()).toString());
//            } catch (Exception e) {
//                channel.basicNack(deliveryTag,false,true);
//                log.warn("信息放入队列{}", DateTime.now());
//                throw new BusinessException(ErrorCode.SYSTEM_ERROR,"AI 服务错误");
